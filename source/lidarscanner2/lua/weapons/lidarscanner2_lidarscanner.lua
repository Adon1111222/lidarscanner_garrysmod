...
local LIDARSCANNER_VERSION = "2.0"
...
if SERVER then
	...
	CreateConVar("lidarscanner_sv_seeker_enabled",1,FCVAR_ARCHIVE,"1 - enables seeker. 0 - disables seeker.",0,1)
...
if CLIENT then
	...
	CreateClientConVar("lidarscanner_burstscan_reverse",0,true,false,"1 - reversed burst scan. 0 - normal burst scan.",0,1)
	...
	"lidarscanner_scansize_sizediff","lidarscanner_scanlines_colour_r","lidarscanner_scanlines_colour_g","lidarscanner_scanlines_colour_b","lidarscanner_background_colour_r","lidarscanner_background_colour_g","lidarscanner_background_colour_b",}
	...
	local function LidarScanner_DoBurstScan(mirrorenabled)
		...
		LidarScanner_SaveScanPos(scanpos,scanang)
		burstscan_pos = burstscan_pos + (RealFrameTime()/2)
	...
	local screeneffect_mat = CreateMaterial("lidarscanner2_lua_materials/screeneffect_material","UnLitGeneric",{["$basetexture"]=render.GetScreenEffectTexture():GetName(),["$basetexturetransform"]="center .5 .5 scale -1 1 rotate 0 translate 0 0",["$ignorez"]=1})
	...
	hook.Add("PostDrawEffects","lidarscanner2_renderscanhook",function()
		...
		if lidarscanner_readqueue then
			local ply = LocalPlayer()
			if lidarscanner_readqueue == -1 then
				if burstscan_pos > 1 then
					curscantype = 0
				end
			elseif lidarscanner_readqueue == 0 then
				LidarScanner_ClearLast()
			elseif lidarscanner_readqueue == 1 then
				curscantype = 1
				LidarScanner_DoScan(ply:EyePos(),ply:EyeAngles(),ply,nil)
			elseif lidarscanner_readqueue == 2 then
			...
			if GetConVarNumber("lidarscanner_showdots") == 1 then
				...
				if updatedots then
					if array_count(dynamic_dots_pos) > 0 then
						...
						local dotsize = GetConVarNumber("lidarscanner_dotsize")
						if IsValid(dynamicmesh) then dynamicmesh:Destroy() dynamicmesh = nil end
				...
				if wep:GetClass() == "lidarscanner2_lidarscanner" then
					...
					if GetConVarNumber("lidarscanner_scansize_scrollinput") == 1 then
						if input_scansize_canscroll then
							...
							--input.SelectWeapon(ply:GetWeapon("lidarscanner2_lidarscanner"))
						end
					end
					cam.Start2D()
					if GetConVarNumber("lidarscanner_hud_enabled") == 1 then
						...
						if input_scansize_canscroll and GetConVarNumber("lidarscanner_scansize_scrollinput") == 1 then
							...
							draw.RoundedBox(3,(ScrW()/2)-(sizew/2),ScrH()*0.9022,sizew,ScrH()/150,Color(255,255,255))--(ScrH()/1.125)+(ScrH()/50)-(ScrH()/150),sizew,ScrH()/150,Color(255,255,255))
